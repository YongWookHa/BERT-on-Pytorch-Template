...
def load_vocab(vocab_file):
    ...
    # for bert special tokens
    for token in ['[CLS]', '[SEP]', '[MASK]']:
        vocab[token] = index
        index += 1
...
class FullTokenizer(object):
    ...
    def __init__(self, config, do_lower_case=True):
        "mode: ['wordpiece', 'bpe']"
        self.basic_tokenizer = BasicTokenizer(do_lower_case=do_lower_case)
        if config.tokenizer == 'bpe':
            self.tokenizer = BytePairEncodingTokenizer(config)
            self.vocab = load_vocab(self.tokenizer.vocab)
        elif config.tokenizer == 'wordpiece':
            self.vocab = load_vocab(config.vocab_file)
...
class BytePairEncodingTokenizer(object):
    """Makes vocab file and runs BPE tokenization."""
    ...
    def __init__(self, config):
        ...
        try:
            ...
            self.sp.Load('{}.model'.format(prefix))
            self.vocab = prefix + ".vocab"
        except Exception:
            raise
